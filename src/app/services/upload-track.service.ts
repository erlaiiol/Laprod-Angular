...
export class UploadTrackService {
   ...
   */
  postTrack(trackData: UploadTrackData): Observable<UploadResponse> {
  ...
  putTrack(trackId: number, trackData: UploadTrackData): Observable<UploadResponse> {
    const formData = new FormData();
    ...
    return this.http.post<UploadResponse>(`${this.apiUrl}/api/tracks/put/${trackId}`, formData);
  }
